        ...
        using (BinaryReader reader = new BinaryReader(fileStream, Encoding.UTF8))
        {
            ...
            for (int i = 0; i < blockCount; i++)
            {
                ...
                for (int j = 0; j < setCount; j++)
                {
                    ...
                    outputBuilder.Append($"{{{vector3.x.ToString(System.Globalization.CultureInfo.InvariantCulture)}," +
					$"{vector3.y.ToString(System.Globalization.CultureInfo.InvariantCulture)}," +
					$"{vector3.z.ToString(System.Globalization.CultureInfo.InvariantCulture)}}}");
                }
