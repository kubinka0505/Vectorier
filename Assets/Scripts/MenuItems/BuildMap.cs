...
--/>";
	...
	void ConvertToTrapezoid(XmlNode node, XmlDocument xml, GameObject trapezoidInScene) {
		...
		// Add X position (Refit into the Vector units)
		T_element.SetAttribute("X", Engine.Math.GameUnits.Multiply(trapezoidInScene.transform.position.x));

		// Add Y position (Negative because Vector sees the world upside down)
		T_element.SetAttribute("Y", Engine.Math.GameUnits.Multiply(-trapezoidInScene.transform.position.y));
		...
		if (spriteRenderer != null && spriteRenderer.sprite != null) {
			...
			} else if (objRegex == "trapezoid_type2") {
				// Mirrored slope
	...
	void ConvertToDynamic(XmlNode node, XmlDocument xml, GameObject dynamicInScene, UnityEngine.Transform dynamicInSceneTransform) {
		...
		foreach (UnityEngine.Transform childObject in dynamicInSceneTransform) {
			if (childObject.gameObject.CompareTag("Object")) {
				...
				// Slope Default
				if (Regex.Replace(childObject.name, globalRegex, string.Empty) == "trapezoid_type1") {
					// Create a new node from scratch
					XmlElement T_element = xml.CreateElement("Trapezoid");
				...
				} else if (Regex.Replace(childObject.name, globalRegex, string.Empty) == "trapezoid_type2") {
