...
public class BackdropPositionController : MonoBehaviour
{
    ...
    private void OnEditorUpdate()
    {
        ...
        if (!isDragging) // Only update position when not dragging
        {
    ...
    private void UpdatePositionAndScaleBasedOnCamera()
    {
        ...
        {
            float scaleFactor = 1 / divisionFactor; // Smaller divisionFactor -> Larger scale
    ...
    private void RestoreInitialTransform()
    {
        if (backdropTransform != null)
        {
            // Restore the object's position and scale to their original values
    ...
    {
        if (backdropTransform == null) return;
