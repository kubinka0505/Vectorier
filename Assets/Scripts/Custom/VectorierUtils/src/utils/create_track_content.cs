...
public class DZMake : MonoBehaviour {
	...
	[MenuItem("Vectorier/⚙ Utils/🗃️ Advanced level build/Marmalade %#G")]
	public static void CreateTrackContent_Marmalade_Invoke_Basic() {
		CreateTrackContent(true, false);
	}

	[MenuItem("Vectorier/⚙ Utils/🗃️ Advanced level build/Marmalade + Run game")]
	public static void CreateTrackContent_Marmalade_Invoke_RunGame() {
		CreateTrackContent(true, true);
	}

	[MenuItem("Vectorier/⚙ Utils/🗃️ Advanced level build/Unity")]
	public static void CreateTrackContent_Unity_Invoke_Basic() {
		CreateTrackContent(false, false);
	}

	[MenuItem("Vectorier/⚙ Utils/🗃️ Advanced level build/Unity + Run game")]
	public static void CreateTrackContent_Unity_Invoke_RunGame() {
		CreateTrackContent(false, true);
	}
	...
	private static string directoryOutputGameName = "Vector"; // "Put to game directory";
	private static string directoryOutputProjectName = "Vectorier"; // "Put to editor directory";

	// Steam game app ID
	private static string gameAppID = "248970";

	// Archive extension supported by 7z CLI. "7z" or "zip" reccomended. Case sensitive!
	private static string fileOutputArchive_Full_Type = "7z";

	// ----- //

	// Variables

	private static bool dummyArchiveMode = false; // after this script is done, restore the level archive that was before
	...
	private static string scenePath = SceneManager.GetActiveScene().path;
	private static string sceneName = Path.GetFileNameWithoutExtension(scenePath);

	// Other paths
	private static string directoryAssetsTextures = Path.Combine(directoryAppdata, "Resources", "Textures");
	private static string directoryTEMP = Path.Combine(Directory.GetParent(Path.GetTempPath()).ToString(), "_Vectorier", "TempBuilder").Replace("/", separator);
	private static string directoryProject = Path.Combine(directoryAppdata, scenePath).Replace("/", separator);

	private static string directoryOutputTop = Path.Combine(
		directoryAppdata,
		"Scenes",
		"z_Exported" // Top directory basename
	).Replace("/", separator);

	private static string directoryOutput = Path.Combine(
		directoryOutputTop,
		sceneName,
	...
	private static string directoryOutputGame = Path.Combine(directoryOutput, directoryOutputGameName).Replace("/", separator);
	private static string directoryOutputProject = Path.Combine(directoryOutput, directoryOutputProjectName).Replace("/", separator);

	// Archive files
	private static string fileOutputArchive_Full = Path.Combine(directoryOutput, sceneName + "." + fileOutputArchive_Full_Type);
	private static string fileOutputArchive_TrackContent = Path.Combine(directoryOutputGame, "track_content_2048.dz").Replace("/", separator);

	static string authorsPlaceholder = "[AUTHORS]";
	static string customTracksDirectoryName = "Custom Levels";

	private static string directoryOutputAssets = Path.Combine(directoryOutputProject, "Assets").Replace("/", separator);
	private static string directoryOutputScenes = Path.Combine(directoryOutputAssets, "Scenes", customTracksDirectoryName, authorsPlaceholder, sceneName);
	private static string directoryOutputAssetsTextures = Path.Combine(directoryOutputAssets, "Resources", "Textures").Replace("/", separator);


	// Temporary directories and files
	private static string directoryWorkingBase = Path.Combine(directoryTEMP, $"!_VectorierUtils_{randomNumber}");
	private static string directoryWorkingImages = Path.Combine(directoryWorkingBase, "Images");
	private static string fileWorkingConfig = Path.Combine(directoryWorkingBase, "config.dcl");

	// -=-=-=- //

	// Executables
	private static string fileExecutable7zip = Path.Combine(Path.GetDirectoryName(cwd), "..", "bin", "7z.exe").Replace("/", separator);
	private static string fileExecutable7zip_Arguments = $"a -t{fileOutputArchive_Full_Type} -mx=9 \"{fileOutputArchive_Full}\" \"{directoryOutput}/*\"".Replace("/", separator);

	private static string fileExecutableDzip = Path.Combine(directoryAppdata, "XML", "dzip", "dzip.exe");
	private static string fileExecutableDzip_Arguments = $"\"{fileWorkingConfig}\" -v";

	private static string fileExecutableExplorer = @"C:/Windows/explorer.exe".Replace("/", separator);
	private static string fileExecutableCMD = @"C:/Windows/System32/cmd.exe".Replace("/", separator);
	...
	void Awake() {
		...
		try {
			buildMap.transparentInterfaceButtons = false;
		} catch {
			// pass
		}

		try {
			string authorsParsed = string.Join(", ", buildMap.authors.Split(',').Select(x => x.Trim()));
			directoryOutputScenes = directoryOutputScenes.Replace(authorsPlaceholder, authorsParsed);
		} catch {
			directoryOutputScenes = directoryOutputScenes.Replace(authorsPlaceholder, "");
		}

		directoryOutputScenes = directoryOutputScenes.Replace("/", separator);

		// Paths
		gamePath = Vectorier.Settings.GameDirectory;
	...
	void CopyWithSubfiles(string sourceFile, string destDir, bool copyMeta, int index) {
		...
		if (!File.Exists(sourceFile)) {
			Debug.Log($@"File cannot be accessed (""{sourceFile}"")");
		} else {
	...
	public static void CreateTrackContent(
		bool buildForMarmalade,
		bool runGameAfterBuild
	) {
		// NotImplemented
		if (!buildForMarmalade) {
			return;
		}
		...
		// Additional files (Game)
		string[] additionalFilesToCopyGame = {
			// Movement
			"animations.dz",
			"Moves_new.xml",

			// Track thumbnail & GUI buttons
			"GUI_2048_1536.dz",
			"track_content_universal.dz",
			"sound.dz",

			// Track name and tricks
			"common_xml.dz"
		};

		foreach (string fileBaseName in additionalFilesToCopyGame) {
			File.Copy(
				Path.Combine(instance.gamePath, fileBaseName),
				Path.Combine(directoryOutputGame, fileBaseName)
			);
		};

		// -=-=-=- //

		// Restore backup (conditional)
		if (dummyArchiveMode) {
		...
		using (StreamWriter writer = new StreamWriter(fileWorkingConfig)) {
			...
			writer.WriteLine($"file \"stopsign.png\" 0 dz");
			writer.WriteLine($"file \"stopsign.plist\" 0 dz");
			writer.WriteLine($"file \"run_indicator.png\" 0 dz");
			writer.WriteLine($"file \"run_indicator.plist\" 0 dz");
			writer.WriteLine($"file \"reverse_indicator_left.png\" 0 dz");
			writer.WriteLine($"file \"reverse_indicator_left.plist\" 0 dz");
			writer.WriteLine($"file \"reverse_indicator_right.png\" 0 dz");
			writer.WriteLine($"file \"reverse_indicator_right.plist\" 0 dz");
			writer.WriteLine($"file \"run_indicator_reverse.png\" 0 dz");
			writer.WriteLine($"file \"run_indicator_reverse.plist\" 0 dz");
		}
