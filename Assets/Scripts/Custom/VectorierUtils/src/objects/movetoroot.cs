...
public class MoveChildrenToRootUtility {
	[MenuItem("Vectorier/⚙ Utils/✨ Move children to root & remove empty parents %#X")]
	private static void MoveChildrenToRoot() {
		GameObject root = Selection.activeGameObject;
		if (root == null) {
			Utils.AdvancedLog("No GameObject selected. Please select a GameObject.");
			return;
		}
