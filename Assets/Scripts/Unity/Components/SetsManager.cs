...
public class SetsManager : MonoBehaviour {
	...
	[HideInInspector]
	public List<int> dropdownSelections = new List<int> { 0, 0, 1 };
...
public class SetsManagerEditor : Editor {
	...
	private string xmlRelativeRoot = Path.Combine("Assets", "XML", "dzip", "level_xml");
				...
				if (absoluteSelected.StartsWith(absoluteXmlRoot)) {
					...
					if (setsManager.tags.Contains(relative)) {
						...
						if (isSwf) {
							...
							// Library index
							setsManager.dropdownSelections[i] = 2;
						}
					}
				} else {
