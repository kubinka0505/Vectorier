...
public class TriggerSettingsEditor : Editor {
	...
	public override void OnInspectorGUI() {
		...
		// Calculate dynamic height based on content lines, with a minimum of 5 lines
		...
		if (GUILayout.Button(refreshButton, GUILayout.Width(25))) {
			LoadTemplatePaths(); // manual refresh
		...
		if (File.Exists(fullPath)) {
			...
			// Optional safety: avoid setting same content
	...
	private void LoadTemplatePaths() {
		...
		if (Directory.Exists(templatesDirectory)) {
			...
			foreach (var path in files.OrderBy(p => p)) {
				...
				// Prettify label: remove extension, split folders and filename
				string noExt = Path.ChangeExtension(relativePath, null); // removes .xml
				string[] parts = noExt.Split('/');
				string label = string.Join(" / ", parts.Select(
					p => System.Threading.Thread.CurrentThread.CurrentCulture.TextInfo.ToTitleCase(p.Replace("_", " ").ToLower()
				)));
