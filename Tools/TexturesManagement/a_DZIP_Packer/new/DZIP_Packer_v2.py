...
__version__ = "2.0"
__date__    = "20th July 2024"
__doc__     = open(__file__, "r", encoding = "UTF-8").readlines()[0].strip('"')[:-4]

#-=-=-=-#
# Variables

showcase_string = f"{__title__} v{__version__}"
...
IMAGE_VARIATIONS = ("json/image_variations.json", "included")
IMAGE_VARIATIONS = json.load(open(IMAGE_VARIATIONS[0], "r", encoding = "UTF-8"))[IMAGE_VARIATIONS[1]]
...
OPTIMIZATION_EXCLUSIONS = ("json/optimization_exclusions.json", "included")
OPTIMIZATION_EXCLUSIONS = json.load(open(OPTIMIZATION_EXCLUSIONS[0], "r", encoding = "UTF-8"))[OPTIMIZATION_EXCLUSIONS[1]]
OPTIMIZATION_EXCLUSIONS = [os.path.normpath(path) for path in OPTIMIZATION_EXCLUSIONS]
...
def transform_images(base_dir: str, excluded: tuple, excluded_directories: tuple, create_subdirs: bool) -> list:
	files_to_process = []
	files_to_transform = []
	plist_files = set()
	...
	for file_path in Path(base_dir).rglob("*"):
		...
		# Store plist files for reference
		if file_path.suffix.lower() == ".plist":
			plist_files.add(os.path.splitext(file)[0])

		# Process only image files
		if os.path.isfile(file_path) and file_path.suffix.lower() in (".png", ".jpg", ".jpeg"):
			base_name_without_ext = os.path.splitext(file)[0]
			
			# Skip images that have a corresponding .plist file
			if base_name_without_ext in plist_files:
				continue
			...
			# Only transform files not in excluded directories
			if not any(excluded_directory in file for excluded_directory in excluded_directories):
				files_to_transform.append(file)

	# Check for duplicated base names
	...
	for file in files_to_process:
		...
		basename_to_fullpaths[basename].append(file)

	print("Searching for duplicates...", end="\r")
	# Find duplicates
	duplicates = {basename: paths for basename, paths in basename_to_fullpaths.items() if len(paths) > 1}

	if duplicates:
		print("The following files have duplicate base names:")

		for basename, paths in duplicates.items():
			print(f'Base name "{basename}" is duplicated in the following files:')
			for path in paths:
				print(f'- "{path}"')

		raise ValueError("Duplicate base names detected. See details above.")
	
	total_files = len(files_to_transform) * len(IMAGE_VARIATIONS.keys())
	paths = []
	processed_files = 0

	for file_path in files_to_process:
		# Skip files in excluded directories during transformation
		if any(excluded_directory in file_path for excluded_directory in excluded_directories):
			continue
		...
		try:
			with Image.open(file_path) as img:
				...
				base_name, base_ext = os.path.splitext(os.path.basename(file_path))

				for suffix, variation in IMAGE_VARIATIONS.items():
					suffix = suffix.strip("_").replace(" ", "_")
					suffix = "_" + suffix

					if create_subdirs:
						save_subdir = os.path.join(save_dir, suffix.lstrip("_"))
						ret_format = ""
					else:
					...
					variation_file_name = ret_format.join((
						base_name.strip("_"),
						base_ext
					))

					# Modify
					modified_img = img.transpose(getattr(Image, variation))
					modified_file_path = os.path.join(save_subdir, variation_file_name)
					...
					modified_img.save(modified_file_path, quality=100)
					...
					if IS_WIN:
						os.system(f"title {showcase_string} ^| {percentage:.2f}% {processed_files_str}")

					_msg = "Processing:"
					ts = os.get_terminal_size().columns - (len(_msg) + 1)
					print(
						_msg, \
...
switch.add_argument(
	...
	action = "store_true",
	help = "Do not create subdirectories for transformed images."
)
...
for fmt in ("PNG", "JPG", "JPEG"):
	for file in Path(args.input_directory_process).rglob("*." + fmt.lower()):
		...
		if not any(exclusion in resolved_file for exclusion in OPTIMIZATION_EXCLUSIONS):
			files_to_optimize.append(resolved_file)
...
# Copying images only
extracted_files = copy_to_dir(args.input_directory_process, None, (("PNG", "JPG", "JPEG", "PLIST")))
